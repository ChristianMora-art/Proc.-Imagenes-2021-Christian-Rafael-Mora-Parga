...
def orientation_map(gray, n):
    ...
    grad_x, grad_x = np.float32(grad_x), grad_x * (1 / 512)
    grad_y, grad_y = np.float32(grad_y), grad_y * (1 / 512)

    # Gradient and smoothing
    grad_x2, grad_y2 = cv2.multiply(grad_x, grad_x), cv2.multiply(grad_y, grad_y)
    grad_xy = cv2.multiply(grad_x, grad_y)
    g_x2, g_y2 = cv2.blur(grad_x2, (n, n)), cv2.blur(grad_y2, (n, n))
